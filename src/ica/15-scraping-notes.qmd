...
extractor <- function(link){
  ...
  article_titles <- article |>
    ...
    html_text()
  Sys.sleep(2)
  date_descriptions <- article |> 
    html_elements(".thumbnail-teaser__description") |> 
    html_text()
  Sys.sleep(2)
  return(data.frame(article_title = article_titles, date_description = date_descriptions))
}


article_df <- data.frame(article_title = character(),
                         date_description = character()
)
for(link in link_list){
  headings <- extractor(link)
  article_df <- bind_rows(article_df, headings)
}
